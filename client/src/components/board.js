...
function getLeftFacingTrianglePath(x, y, width, height) {
  return Snap.format(
    "M {centerLeft} {y} L {right} {bottom} L {right} {top} Z",
    {
      x: x,
      y: y,
...
export default function Board({ board, player, sendPlayerMove }) {
  ...
  useEffect(() => {
    ...
    board?.board.map((row, rowIndex) => {
      const leftButton = s.path(
        getRightFacingTrianglePath(topOffset, 80 * rowIndex + topOffset, 30, 40)
      );
      ...
      const rightButton = s.path(
        getLeftFacingTrianglePath(
          topOffset + 640,
